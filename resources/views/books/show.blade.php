    ...
    <div class="w-full flex justify-center mt-6">
            ...
            <div class="flex justify-center h-32 items-center">
                <div class="py-2 mt-4 w-10/12 bg-gray-200 flex justify-center">
                    @foreach ($book->medias as $media)
                        <img class="px-1 w-24 h-24" src="{{ asset($media->link) }}"">
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class=" w-5/12 bg-white min-h-fit p-6 flex flex-col rounded-lg">
                    ...
                    <div class="flex flex-col pt-1">
                        ...
                        <p class="ml-3 text-lg font-semibold">{{ $book->description }}
                        </p>
                    </div>
                    <div>
            ...
            <div class="w-full flex justify-center mt-6">
                <div class="w-9/12 pt-6 bg-white min-h-fit p-6 rounded-lg">
                    ...
                    @foreach ($book->reviews as $review)
                        <div class="flex flex-col ml-5 pb-4 h-40 border-b-4 border-gray-500">
                            <div class="flex items-center">
                                <img class="w-12 h-12 rounded-full" src="{{ asset('/storage/user1.jpg') }}" alt="">
                                <label class="ml-4 text-xl will-change-transformtext-lg font-bold">{{ $review->user->first_name . ' ' . $review->user->last_name }}</label>
                            </div>
                            <p class="pl-16"> {{ $review->created_at }}</p>

                            <div class="pl-16 mb-2 flex">
                                @for ($i = 1; $i <= config('app.book_rating'); $i++) @if ($i <=$review->rating) <svg class="w-4 h-4 text-yellow-500 " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                    ...
                    <form action="{{ route('reviews.store') }}" method="POST">
                        ...
                        <div class="flex flex-col mt-6">
                            ...
                            <x-button class="mt-3 w-24 h-12 flex justify-center bg-gray-600"> {{ __('Send') }}
                            </x-button>
                            <div>
                            </div>
                        </div>
